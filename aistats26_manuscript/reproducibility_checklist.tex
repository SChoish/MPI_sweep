...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \textbf{Yes.} Section~\ref{sec:method} states that PART uses $K$ persistent actors and $K$ actor optimizer calls per delayed update; Eq.~\eqref{eq:margin}, Proposition~\ref{prop:exposure}, and the local-idealization subsection state the theoretical properties and scope. The supplement reports a verified one-stack time and memory profile.
    ...
    \textbf{Partial.} The anonymized release contains the PART launch, evaluation, diagnostic, and verification code plus the compact policy-separation audit; historical \texttt{BAR} identifiers are retained in paths and configuration keys for traceability. The control-run configs identify local training-source files by SHA-256, but did not record a clean Git revision.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \textbf{No.} All recoverable settings are listed, but exact per-run PART-P4 configurations and manifests for proximal seeds 2--3 are unavailable; the policy-separation configs also omit a source revision and environment lock.
    ...
    \textbf{No.} The released artifact fully records the H200 used for the actor-cost profile and the policy-separation CPU recovery chain, but does not contain a complete, independently verified inventory of all training hardware.
  \end{enumerate}
