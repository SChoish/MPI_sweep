...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \textbf{Yes.} Section~\ref{sec:method} states that BAR uses $K$ persistent actors and $K$ actor optimizer calls per delayed update; the theoretical properties are stated in Propositions~\ref{prop:margin}--\ref{prop:local_composition}.
    ...
    \textbf{Yes.} The supplementary material specifies the implementation and dependencies; the anonymized release contains launch, evaluation, diagnostic, and verification scripts.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \textbf{Yes.} The fixed budget grid, datasets, seeds, update counts, actor schedule, evaluation protocol, and targeted-control protocols are specified in Section~\ref{sec:experiments} and the supplementary material.
    ...
    \textbf{Yes.} Captions and text define seed-mean cells, raw seeded runs, collapse thresholds, paired ratios, and the targeted four-seed mean and sample standard deviation. The main grid is explicitly identified as a two-seed descriptive phase diagram.
    ...
    \textbf{No.} The released artifact records software and execution protocols but does not contain a complete, independently verified inventory of all external-machine hardware.
  \end{enumerate}
