#!/usr/bin/env python3
"""Check a trusted local shared-driver checkpoint pair and final eval rows.
...
def run_dir(save_dir: Path, env: str, tau: float, seed: int, branch: str) -> Path:
    return save_dir / f"{env}_tau{tau:g}_shared_{branch}3_seed{seed}"
...
def read_final_score(path: Path, step: int) -> tuple[float, float]:
    ...
    return float(rows[0]["d4rl_score"]), float(rows[0]["d4rl_pi3"])
...
def verify_pair(save_dir: Path, env: str, tau: float, seed: int, step: int) -> dict:
    ...
    for branch, checkpoint in checkpoints.items():
        ...
        if len(checkpoint["actors_steps"]) != 3 or any(
    ...
    return {
        "status": "pair_verified", "env": env, "T": tau, "K": 3,
